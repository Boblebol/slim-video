...
def test_format_report_delete_original(tmp_path: Path) -> None:
    summary = BatchSummary(
        directory=tmp_path,
        ...
        total_original_bytes=2_000_000_000,
        total_final_bytes=1_000_000_000,
        ...
        records=[
            TranscodeRecord(
                ...
                gain_pct=50.0,
                saved_bytes=1_000_000_000,
                elapsed_seconds=100.0,
                speed="10.0x",
                output_path=str(tmp_path / "movie.hevc.mkv"),
    ...
    report_text = format_report(summary)
    assert "DELETED directly after encode" in report_text
    assert "Original files WERE permanently deleted" in report_text
